...
import struct
import unittest
from utils import array as uarray
...
class ArrayTest(unittest.TestCase):
    ...
    def test_to_ctypes(self):
        for seq, dtype in ((singlebyteseq, ctypes.c_ubyte),
            ...
            bytebuf, size = uarray.to_ctypes(seq, dtype)
    ...
    def test_CTypesView__singlebytes(self):
        buf1 = uarray.CTypesView(singlebyteseq, docopy=True)
        buf2 = uarray.CTypesView(singlebytebuf, docopy=False)
        for singlebytes, shared in ((buf1, False), (buf2, True)):
            self.assertIsInstance(singlebytes, uarray.CTypesView)
    ...
    def test_CTypesView__doublebytes(self):
        buf1 = uarray.CTypesView(doublebyteseq, USHORT_SIZE, docopy=True)
        buf2 = uarray.CTypesView(doublebytebuf, USHORT_SIZE, docopy=False)
        for singlebytes, shared in ((buf1, False), (buf2, True)):
            self.assertIsInstance(singlebytes, uarray.CTypesView)
            # Pypy 1.8.0 does not support ctypes.from_buffer(), hence we
            # never will receive a shared one
    ...
    def test_CTypesView__quadbytes(self):
        buf1 = uarray.CTypesView(quadbyteseq, UINT_SIZE, docopy=True)
        buf2 = uarray.CTypesView(quadbytebuf, UINT_SIZE, docopy=False)
        for singlebytes, shared in ((buf1, False), (buf2, True)):
            self.assertIsInstance(singlebytes, uarray.CTypesView)
            # Pypy 1.8.0 does not support ctypes.from_buffer(), hence we
            # never will receive a shared one
    ...
    def test_CTypesView__repr__(self):
        ...
        for seq, size, factor, shared in seqs:
            buf = uarray.CTypesView(seq, size, not shared)
    ...
    def test_MemoryView(self):
        self.assertRaises(TypeError, uarray.MemoryView, 5, 1, (1,))
        self.assertRaises(TypeError, uarray.MemoryView, None, 1, (1,))

        source = "Example buffer"
        view = uarray.MemoryView(source, 1, (len(source),))
        for index, val in enumerate(view):
            self.assertEqual(val, source[index])

        view = uarray.MemoryView(source, 1, (2, 7))
    ...
    def test_MemoryView_ndim_strides(self):
        ...
        view = uarray.MemoryView(source, 1, (7, 2))
        self.assertEqual(view.ndim, 2)
        self.assertEqual(view.strides, (7, 2))
        view = uarray.MemoryView(source, 1, (2, 2, 2))
    ...
    def test_MemoryView_itemsize(self):
        ...
        view = uarray.MemoryView(source, 7, (1, 7))
    ...
    def test_create_array(self):
        ...
        for i in (1, 2, 4, 8):
            parr = uarray.create_array(barr, i)
        ...
        for i in (0, 3, 5, 6, 7, 9, 10, 12, "test", self):
            self.assertRaises(TypeError, uarray.create_array, barr, i)
